...
from tensorflow.keras.applications.resnet50 import ResNet50, preprocess_input as pre_in_resnet
from tensorflow.keras.applications.inception_v3 import InceptionV3, preprocess_input as pre_in_inception
from tensorflow.keras.applications.xception import Xception, preprocess_input as pre_in_xception # TensorFlow ONLY
from tensorflow.keras.applications.vgg16 import VGG16, preprocess_input as pre_in_vgg16
from tensorflow.keras.applications.vgg19 import VGG19, preprocess_input as pre_in_vgg19
...
class Files:
    def __init__(self,dataset_path, n_classes):
        ...
        self.dataset = []
        print('Indexing all the files in dataset %s' %self.dataset_path)
        self.do_index()
        print('done.\n\n')
    ...
    def get_file_path(self, file_id):
        image_file_path = 'File not found'
        text_file_path = 'File not found'
        index = self.find(self.dataset, 'file_id', file_id)
        if index != -111:
            image_file_path = os.path.join(self.dataset_path,
                                     str(self.dataset[index].get('class_id')),
                                     "img_"+file_id+'.'+self.dataset[index].get('img_ext'))
            text_file_path = os.path.join(self.dataset_path,
                                           str(self.dataset[index].get('class_id')),
                                           "descr_"+file_id + '.' + self.dataset[index].get('text_ext'))
